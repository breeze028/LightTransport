...
### 11.7 CUDA wavefront 的 BVH8 原型

编辑器和 CLI 的加速结构设置现在是显式的：`Flat BVH` 或 `Two-level BVH`。旧的 `Auto` 模式已经移除，因为它会让 wavefront 的实际行为变得含糊。默认值是 `Two-level BVH`；`--accel auto` 会被当作未知值处理，并保持默认设置不变。

CUDA wavefront 现在把 wide traversal layout 视为 renderer 内部细节。编辑器只暴露 `CUDA Megakernel Path Tracer` 和 `CUDA Wavefront Path Tracer` 作为渲染器变体，不把 BVH8 或 CWBVH 暴露成用户可选的加速结构模式。当编辑器选中 CUDA Wavefront 时，`Acceleration` 固定为 `Two-level BVH (Wavefront internal)`。

现有二叉 BVH 和 compact binary traversal 仍会上传，作为正确性基线和 flat-path fallback。BVH8 数据通过折叠已有二叉 BLAS root 构建，每个 wide node 最多 8 个 child；TLAS 仍保持二叉。它继续作为 full-float wide-node baseline 上传。

CWBVH v1 已经接入为 CUDA wavefront + two-level BLAS 的下一种内部布局。它使用受 tinybvh / Ylitie 等工作启发的 80 字节压缩 wide node、独立的 CWBVH 叶三角形表，并复用现有 `GpuTraversalTriangle` 命中路径。CWBVH 不新增编辑器或 CLI 选项；如果某个 mesh 没有 CWBVH root，遍历会 fallback 到 BVH8，再 fallback 到二叉 BLAS。只有当 Sponza median frame time 至少提升 5%，且 P95 没有回退时，它才应该成为优先默认路径。

更完整的布局、打包和验证说明见 [13.5-cwbvh.md](13.5-cwbvh.md)。
