...
### 11.7 BVH8 prototype for CUDA wavefront

The editor and CLI acceleration setting is now explicit: `Flat BVH` or `Two-level BVH`.
The old `Auto` mode was removed because it made wavefront behavior ambiguous.
The default is `Two-level BVH`; `--accel auto` is treated as an unknown value and leaves the default unchanged.

CUDA wavefront now treats the wide traversal layout as an internal renderer detail.
The editor exposes `CUDA Megakernel Path Tracer` and `CUDA Wavefront Path Tracer` as renderer variants, but it does not expose BVH8 or CWBVH as user-facing acceleration modes.
When CUDA Wavefront is selected in the editor, `Acceleration` is fixed to `Two-level BVH (Wavefront internal)`.

The existing binary BVH and compact binary traversal remain uploaded as the correctness baseline and as the flat-path fallback.
The BVH8 data is built by collapsing the existing binary BLAS roots into up to eight children per wide node; TLAS remains binary.
It remains uploaded as the full-float wide-node baseline.

CWBVH v1 is now wired as the next internal layout for CUDA wavefront + two-level BLAS.
It uses an 80-byte compressed wide node inspired by tinybvh / Ylitie et al., a separate CWBVH leaf-order triangle-index table, and the existing `GpuTraversalTriangle` hit path.
CWBVH does not add an editor or CLI option; if a mesh has no CWBVH root, traversal falls back to BVH8 and then binary BLAS.
It should only become the preferred default if Sponza median frame time improves by at least 5% without P95 regression.
